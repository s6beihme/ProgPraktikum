...
template<typename T>
void read_parameters_from_file(std::string filename, int& imax, int& jmax, T& xlength, T& ylength, T& delt, T& t_end, T& tau, T& del_vec, T& eps, T& omg, T& alpha, int& itermax, T& GX, T& GY, int& Re, T& UI, T& VI, T& PI){
	std::ifstream myfile;
...
template <typename T>
void calculate_F_G(int imax,int jmax,T delt,T delx,T dely, int Re, T alpha,T GX,T GY, std::unique_ptr<std::unique_ptr<T[]>[]>& U, std::unique_ptr<std::unique_ptr<T[]>[]>& V, std::unique_ptr<std::unique_ptr<T[]>[]>& F, std::unique_ptr<std::unique_ptr<T[]>[]>& G) {
...
//pass P2, so it doesnt have to be created each time function is used
template<typename T>
